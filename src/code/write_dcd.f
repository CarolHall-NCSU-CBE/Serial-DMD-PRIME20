	...
	do k=1,nop1/numbeads1+nop2/numbeads2
		...
		n = (k-1)*numbeads1+1			  
 			do while (j .le. chnln1)								
				write(trajdcd) xtmp(n+chnln1),ytmp(n+chnln1),ztmp(n+chnln1)								
				write(trajdcd) xtmp(n),ytmp(n),ztmp(n)					
				write(trajdcd) xtmp(n+2*chnln1),ytmp(n+2*chnln1),ztmp(n+2*chnln1)				
   	  			if(j.ne.chnln1) then
   	  			...
   	  			write(trajdcd) rx1,ry1,rz1								
				if (fside1(j).ne.0) then
				  	write(trajdcd) xtmp(n-p+3*chnln1),ytmp(n-p+3*chnln1),ztmp(n-p+3*chnln1)								  
					m = m + 5
				else 
					p = p + 1							
					m = m + 4
			...
			do while (j .le. chnln2)																
                  		write(trajdcd) xtmp(n+chnln2),ytmp(n+chnln2),ztmp(n+chnln2)								
				write(trajdcd) xtmp(n),ytmp(n),ztmp(n)					
				write(trajdcd) xtmp(n+2*chnln2),ytmp(n+2*chnln2),ztmp(n+2*chnln2)				
   	  			if(j.ne.chnln2) then
				...
				if (fside2(j).ne.0) then
					write(trajdcd) xtmp(n-p+3*chnln2),ytmp(n-p+3*chnln2),ztmp(n-p+3*chnln2)							  
					m = m + 5
				else 
					p = p + 1							
					m = m + 4
